...
| jarvis5.0 (rev21) | **Settings/wizard parity + agent consistency refresh (v1.3.1)**: shipped inline Settings editor + chain/service tree controls + per-service removal + legacy chain TOML field parity import/apply bridge + `features.mask_rpc` rewrite support (`a69ee02`); project memory save appended (`agents/projects/vprox.state.md`); agent definitions audited for stale push references — reviewer updated to fleet paths/config, jarvis5.0 fleet section fixed (`PushDefaults` rename, POST delete alias), and resolved findings reflected (`openFleetDB` + `RemoveRegisteredChain`); skills/resources last-updated markers bumped to rev21 with current capability notes. |
| jarvis5.0 (rev22) | **Agentupgrade + v1.4.0 kickoff**: 8 new awesome-copilot skills installed to `.github/skills/` — `gh-cli` (full gh reference), `architecture-blueprint-generator` (arch docs from codebase), `create-architectural-decision-record` (ADR for vOps/config/theme decisions), `github-issues` (create/manage GitHub issues + references/), `git-flow-branch-creator` (Git Flow branch creation), `generate-custom-instructions-from-codebase` (migration instructions for vLog→vOps rename), `cloud-design-patterns` (42 patterns + 9 reference files; circuit-breaker/rate-limit/bulkhead directly applicable to vProx), `agentic-eval` (evaluator-optimizer patterns for agent QA); v1.4.0 branch `vLog_v1.4.0` active; vOps rename scope confirmed (5 external importers, packages already generically named); v1.4.0 phases queued: vOps rename → CLI merger + completion → config P1-P4 → themes + prefs → wizard parity |
| jarvis5.0 (rev23) | **Agentupgrade + pluginsupgrade (v1.4.0 delivery)**: 3 new skills installed — `doublecheck` (3-layer AI output verification pipeline, bundled `assets/verification-report-template.md`; use when QA-ing agent-generated code or design decisions), `model-recommendation` (analyzes chatmode/prompt files and recommends optimal models by task complexity/cost; use when configuring agent chains), `agent-governance` (governance/safety controls for multi-agent workflows; use when orchestrating sub-agents with elevated privileges); all 14 previously installed skills verified ✅ current against remote (no updates needed); 3 new patterns added to `base.agent.md` — CSS variable theming architecture (`--vn-*` tokens, 3 theme blocks), smart input widths via CSS attribute selectors, Makefile cross-compile + deploy-jarvis pattern; **v1.4.0 delivery summary**: all 6 phases complete (vOps rename, CLI merge/completion, config P1-P4, 3-theme system, wizard parity, security audit fixes — SSRF/RCE/SSH/brute-force/spoof all resolved); `cmd/vops/main.go version="1.4.0"`, default theme `light-blue`, binary `vops` serving at jarvis:18889; branch `vLog_v1.4.0` at `1a9c45b`; next: live QA on jarvis testnet → merge to main → tag v1.4.0 |
