...
- **Push/SSH pattern**: Use `golang.org/x/crypto/ssh` client with a **dedicated key** for push→VM connections (separate from `id.file`/operator keys). Validate VM hostname/IP before dialing. Remote exec via SSH channel: `session.CombinedOutput("bash ~/vProx/scripts/chains/{chain}/{component}/{script}.sh")`. Never embed script content; scripts live in repo at `scripts/chains/{chain}/{component}/{script}.sh`.
